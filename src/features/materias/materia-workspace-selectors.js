...
import { calculateTemaProgress } from '../../domain/services/progress-service.js';
...
export function selectTemaSections(data, materiaId) {
  return selectTemasByMateria(data, materiaId).map((tema, index, temas) => {
    ...
    return Object.freeze({
      tema,
      assuntos,
      progress: calculateTemaProgress(data, tema.id),
      canMoveUp: index > 0,
...
export function selectTemaDeleteImpact(data, temaId) {
  ...
  return Object.freeze({
    tema,
    assuntos: selectAssuntosByTema(data, temaId).length,
  });
}
