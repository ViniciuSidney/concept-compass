...
export function createTemaDeleteDialog(
  ...
  impactItem.textContent = `${assuntosCount} ${assuntosCount === 1 ? 'assunto' : 'assuntos'}`;
  ...
  const modal = createModal(documentObject, {
    ...
    description:
      assuntosCount > 0
        ? 'Todos os assuntos relacionados também serão removidos. Essa ação não pode ser desfeita.'
